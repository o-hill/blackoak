'''
    ...
    for teaching an agent to play tetris.

    Mainly sourced from a John Schulmann lecture:
    http://rl-gym-doc.s3-website-us-west-2.amazonaws.com/mlss/lab1.html

    It works! Reformatted for python 3.6.
...
for iteration in range(n_iter):
    # Sample parameter vectora

    # Grab samples from a gaussian distribution.
    thetas = [np.random.normal(mu, sig, dim_theta) for mu, sig in zip(theta_mean, theta_std)]
    ...
    theta_mean = np.array([np.mean(theta) for theta in thetas])
    theta_std = np.array([np.std(theta) for theta in thetas])

    # Used for Mountain Car (based on Szita/Lorinz 2006).
    # theta_std = np.array([np.std(theta) + c for theta in thetas])
    c += 0.03125
